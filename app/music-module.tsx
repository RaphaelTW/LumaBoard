...
export function MusicModule({ onToast }: { onToast: (message: string) => void }) {
  ...
  const loadGenre = async (genre = cache.genre) => {
    if (!hasExternalContentConsent()) {
      setError("Conteúdo externo desativado. Reative em Privacidade para buscar músicas e rádios.");
      return;
    }
    setLoading(true);
    setError("");
  ...
  return (
    <section className="module-view music-view">
      <header className="module-heading">
        ...
        <div className="module-actions"><button className="button primary" onClick={() => void loadGenre()} disabled={loading || !externalContentAllowed}><RefreshCw className={loading ? "spin" : ""} /> {loading ? "Buscando…" : "Atualizar gênero"}</button></div>
      </header>
