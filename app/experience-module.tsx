...
type SystemNotice = { id: string; type: string; title: string; detail: string; action?: () => void };
...
export function ExperienceModule({ summary, publicStatus, weatherStatus, onToast }: { summary: PublicSummary | null; publicStatus: PublicStatus; weatherStatus: string; onToast: (message: string) => void }) {
  ...
  const [inboxFilter, setInboxFilter] = useState<"unread" | "all" | "history">("unread");
  ...
  }, [agenda.overdueTasks, clientErrors, dismissed.audioErrors, dismissed.dismissed, publicStatus, pwa.applyUpdate, pwa.updateAvailable, rainHistory, savedNewsCount, storageIssues, summary?.warnings, weatherStatus]);

  const inboxItems = useMemo(() => {
    if (inboxFilter === "history") return agenda.notificationHistory;
    if (inboxFilter === "unread") return agenda.notificationInbox.filter((item) => !item.readAt);
    return agenda.notificationInbox;
  }, [agenda.notificationHistory, agenda.notificationInbox, inboxFilter]);
  ...
  const activateAndTest = async () => {
    const ok = await agenda.testNotification();
    onToast(ok ? "Notificação de teste enviada." : "O navegador não concedeu permissão para notificações.");
  };
  ...
  return (
    <section className="module-view experience-view">
      ...
      <header className="module-heading"><div><span className="eyebrow">PWA, CALENDÁRIO & NOTIFICAÇÕES</span><h1>Alertas úteis, sem conta e sem nuvem.</h1><p>Caixa de entrada local, histórico, adiamento, horário silencioso e controle completo das permissões.</p></div><div className="module-actions">{pwa.installAvailable && <button className="button primary" onClick={() => void pwa.install()}><Download /> Instalar app</button>}<button className="button secondary" onClick={() => void pwa.checkForUpdate()}><RefreshCw /> Procurar atualização</button></div></header>
      ...
      <section className="pwa-status-grid">
        ...
        <article className="panel pwa-status-card"><span className="metric-icon"><Save /></span><div><strong>{formatBytes(usage.bytes)}</strong><span>{usage.items} áreas locais</span><small>Agenda, temas, layouts, notificações e caches ficam somente neste navegador.</small></div><button className="button secondary" onClick={() => { pwa.createSafetySnapshot(); onToast("Cópia de segurança temporária criada."); }}>Proteger agora</button></article>
      </section>

      <section className="notification-settings-grid">
        <article className="panel notification-settings-card">
          <header><Bell /><div><strong>Alertas do navegador</strong><span>{agenda.notificationPermission === "granted" ? "Permissão concedida" : agenda.notificationPermission === "denied" ? "Permissão bloqueada" : agenda.notificationPermission === "unsupported" ? "Não suportado" : "Aguardando permissão"}</span></div></header>
          <label className="check-row"><input type="checkbox" checked={agenda.notificationSettings.browserNotifications} onChange={(event) => agenda.updateNotificationSettings({ browserNotifications: event.target.checked })} /> Exibir alertas enquanto o LumaBoard estiver aberto</label>
          <button className="button primary" onClick={() => void activateAndTest()}>Ativar e testar</button>
        </article>
        <article className="panel notification-settings-card">
          <header><History /><div><strong>Horário silencioso</strong><span>Os itens entram na central, mas sem popup do navegador.</span></div></header>
          <label className="check-row"><input type="checkbox" checked={agenda.notificationSettings.quietHoursEnabled} onChange={(event) => agenda.updateNotificationSettings({ quietHoursEnabled: event.target.checked })} /> Ativar horário silencioso</label>
          <div className="two-columns"><label>Início<input type="time" value={agenda.notificationSettings.quietStart} onChange={(event) => agenda.updateNotificationSettings({ quietStart: event.target.value })} /></label><label>Fim<input type="time" value={agenda.notificationSettings.quietEnd} onChange={(event) => agenda.updateNotificationSettings({ quietEnd: event.target.value })} /></label></div>
        </article>
        <article className="panel notification-settings-card">
          <header><RefreshCw /><div><strong>Adiamento padrão</strong><span>Usado pelos botões “Adiar” da agenda e da central.</span></div></header>
          <label>Adiar por<select value={agenda.notificationSettings.defaultSnoozeMinutes} onChange={(event) => agenda.updateNotificationSettings({ defaultSnoozeMinutes: Number(event.target.value) })}><option value="5">5 minutos</option><option value="10">10 minutos</option><option value="15">15 minutos</option><option value="30">30 minutos</option><option value="60">1 hora</option></select></label>
          <label>Manter histórico<select value={agenda.notificationSettings.keepHistoryDays} onChange={(event) => agenda.updateNotificationSettings({ keepHistoryDays: Number(event.target.value) })}><option value="7">7 dias</option><option value="30">30 dias</option><option value="90">90 dias</option><option value="365">1 ano</option></select></label>
        </article>
      </section>

      <section className="notification-inbox-section">
        <header className="section-heading"><div><span className="eyebrow">CAIXA DE ENTRADA LOCAL</span><h2>{agenda.unreadNotificationCount} notificações não lidas</h2><small>O histórico e os adiamentos ficam somente neste aparelho.</small></div><div className="section-heading-actions"><button className="text-button" onClick={agenda.markAllNotificationsRead}>Marcar todas como lidas</button><button className="text-button" onClick={() => { if (window.confirm("Limpar todo o histórico local de notificações?")) agenda.clearNotificationHistory(); }}>Limpar histórico</button></div></header>
        <div className="notification-tabs"><button className={inboxFilter === "unread" ? "active" : ""} onClick={() => setInboxFilter("unread")}>Não lidas</button><button className={inboxFilter === "all" ? "active" : ""} onClick={() => setInboxFilter("all")}>Ativas</button><button className={inboxFilter === "history" ? "active" : ""} onClick={() => setInboxFilter("history")}>Histórico</button></div>
        <div className="notification-inbox-list">
          {inboxItems.length === 0 && <article className="panel empty-notifications"><Check /><div><strong>Nenhuma notificação nesta visualização</strong><span>Novos lembretes da agenda aparecerão aqui.</span></div></article>}
          {inboxItems.map((item) => <article className={`panel notification-inbox-item severity-${item.severity} ${item.readAt ? "read" : "unread"} ${item.dismissedAt ? "dismissed" : ""}`} key={item.id}><span className="notification-type">{item.source === "agenda" ? "Agenda" : "Sistema"}</span><div><strong>{item.title}</strong><span>{item.body}</span><small>{formatNotificationDate(item.createdAt)}{item.snoozedUntil ? ` · adiado até ${formatNotificationDate(item.snoozedUntil)}` : ""}{item.dismissedAt ? " · dispensada" : ""}</small></div><div className="notification-item-actions">{!item.readAt && <button className="button secondary" onClick={() => agenda.markNotificationRead(item.id)}>Lida</button>}{!item.dismissedAt && <button className="button secondary" onClick={() => agenda.snoozeNotification(item.id)}>Adiar</button>}{!item.dismissedAt && <button className="icon-button compact" aria-label="Dispensar" onClick={() => agenda.dismissNotification(item.id)}>×</button>}</div></article>)}
        </div>
      </section>

      <section className="notification-center-section">
        <header className="section-heading"><div><span className="eyebrow">AVISOS DO SISTEMA</span><h2>{systemNotices.length} itens que pedem atenção</h2><small>{dismissed.dismissed.length} avisos dispensados permanecem somente neste navegador.</small></div><div className="section-heading-actions">{dismissed.dismissed.length > 0 && <button className="text-button" onClick={() => { const next = { ...dismissed, dismissed: [] }; setDismissed(next); saveDismissed(next); }}>Restaurar dispensados</button>}{systemNotices.length > 0 && <button className="text-button" onClick={() => { const next = { ...dismissed, dismissed: [...dismissed.dismissed, ...systemNotices.map((item) => item.id)].slice(-500) }; setDismissed(next); saveDismissed(next); }}>Dispensar todos</button>}</div></header>
        <div className="notification-center-list">
          {systemNotices.length === 0 && <article className="panel empty-notifications"><Check /><div><strong>Tudo em ordem</strong><span>Nenhuma tarefa vencida, falha ou atualização pendente.</span></div></article>}
          {systemNotices.map((notice) => <article className="panel notification-center-item" key={notice.id}><span className="notification-type">{notice.type}</span><div><strong>{notice.title}</strong><span>{notice.detail}</span></div>{notice.action && <button className="button primary" onClick={notice.action}>Resolver</button>}<button className="icon-button compact" aria-label="Dispensar" onClick={() => dismissSystemNotice(notice.id)}>×</button></article>)}
        </div>
      </section>
