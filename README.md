...
## Privacidade, termos e armazenamento local na v1.8.9
...
## Destaques da versão 1.8.9

- botão **Atualizar agora** no desktop usa o service worker em espera rastreado pelo gerenciador PWA;
- aviso de atualização centralizado no banner global para não aparecer duplicado no desktop;
- testes ignoram `dist/**` para não executar releases empacotadas antigas;
- regressões de segurança ajustadas para dashboards acima do limite e respostas públicas com headers;
- validação aprovada com security scan, lint, testes, build e audit de produção.
...
- `LumaBoardApp.tsx` reduzido com `OverviewModule` e hooks dedicados;
- `/api/public/summary` dividido em módulos por domínio de API pública;
- tipos e helpers compartilhados para fetch, normalização e payloads;
...
- fontes públicas de conteúdo separadas da rota `/api/public/summary`;
- service worker sem parsing de HTML gerado pelo Next.js;
- testes comportamentais para a fonte do Art Institute of Chicago;
- teste de cache PWA para impedir retorno do parsing frágil de assets;
- cache PWA atualizado para distribuir a versão 1.8.3.

## Destaques da versão 1.8.2

- avatar do cabeçalho configurável por iniciais, salvo no `localStorage`;
- nova fonte pública sem chave: obra em domínio público do Art Institute of Chicago;
- fonte escolhida a partir do repositório `public-apis/public-apis`;
- versão do app centralizada para PWA, Functions e User-Agent;
- mais preferências locais passam pelo gerenciador de armazenamento com diagnóstico;
